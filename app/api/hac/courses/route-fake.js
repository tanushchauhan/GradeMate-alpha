...
export async function POST(req, res) {
  const { username, password, options } = await req.json();
  let periodNumber, storeData;
  let data = {};
  let studentName;
  ...
  try {
    ...
    if (options.onlyPeriod) {
      ...
      //   data[u]["weightDetails"] = classes[data[u].courseCode];
      // }

      data = [
        {
          courseCode: "CATE00310",
          ...
          assignmentData: [
            ...
            ["02/15/2024", "3243233", "another", "w", 99, 100],
          ],
          weightDetails: {
            name: "Livestock Production",
            weight: 5.0,
      ...
      for (let i = 1; i < options.limGPA; i++) {
        ...
        if (periodNumber === 2) {
          storeData[0].studentGrade = 95;
        }

        // for (let u = 0; u < storeData.length; u++) {
        //   if (storeData[u].noWeight) {
        //     storeData[u]["weightDetails"] = {
        //       ...classes[storeData[u].courseCode],
        //       weight: 0,
        //     };
        //   } else {
        //     storeData[u]["weightDetails"] = classes[storeData[u].courseCode];
        //   }
        // }
